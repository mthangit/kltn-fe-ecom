...
export function getErrorMessage(error: any): string {
  ...
  if (error.response) {
    ...
    // If detail is an object with message or error_code
    if (detail && typeof detail === 'object') {
      if (detail.message) return detail.message;
      if (detail.error_code) return `Lỗi: ${detail.error_code}`;
    }

    // Check for message directly in data
    if (data?.message) {
      return data.message;
    }

    // Check for error directly in data
    if (data?.error) {
      return data.error;
    }
    ...
    if (status === 400) return detail?.message || 'Yêu cầu không hợp lệ';
